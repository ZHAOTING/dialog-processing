...
if __name__ == "__main__":
    ...
    if config.model in ["hred_student"]:
        model = Model(config, tokenizer, None)
    else:
    ...
    for _ in tqdm(range(math.ceil(len(test_data_source)/config.eval_batch_size))):
        ...
        for _ in range(config.n_sample_times):
    ...
    if config.save_samples:
        ...
        for idx, (ctx, ref, hyp, mhyp) in enumerate(zip(ctxs, refs, hyps, mhyp_texts)):
            sample_json = {
                "sample_idx": idx,
                "context": [],
                "reference": None,
            ...
            sample_json["multi_hypotheses"] = mhyp
    ...
    if config.n_sample_times > 1:
        # Multi-hypotheses extrinsic evaluation
        mhyp_ctxs = []
        mhyp_hyps = []
        mhyp_refs = []
        for idx in range(len(ctxs)):
            n_hyps = len(mhyps[idx])
            for hyp_idx in range(n_hyps):
                mhyp_ctxs.append(ctxs[idx])
                mhyp_hyps.append(mhyps[idx][hyp_idx])
                mhyp_refs.append(refs[idx])
        mhyp_hyp_texts = [text for (text, floor) in mhyp_hyps]
        mhyp_ref_texts = [text for (text, floor) in mhyp_refs]
        # BLEU
        bleu_scores = metrics.batch_bleu(mhyp_hyp_texts, mhyp_ref_texts)
        bleu = np.mean(bleu_scores)
        # Embedding similarities
        avg_emb_sims, ext_emb_sims, greedy_emb_sims = metrics.batch_sim_bow(mhyp_hyp_texts, mhyp_ref_texts)
        avg_emb_sim = np.mean(avg_emb_sims)
        ext_emb_sim = np.mean(ext_emb_sims)
        greedy_emb_sim = np.mean(greedy_emb_sims)
        # SIF embedding similarity
        sif_emb_sims = metrics.batch_sif_emb_sim(mhyp_hyp_texts, mhyp_ref_texts)
        sif_emb_sim = np.mean(sif_emb_sims)
        # Distinct n-grams
        intra_dist1, intra_dist2, inter_dist1, inter_dist2, \
            intra_types1, intra_types2, inter_types1, inter_types2 \
            = metrics.batch_div_distinct(mhyp_hyp_texts)
        # Average sentence length
        hyp_tokens_lst = [eval_tokenizer.convert_string_to_tokens(sent) for sent in mhyp_hyp_texts]
        hyp_lens = [len(tokens) for tokens in hyp_tokens_lst]
        avg_len = np.mean(hyp_lens)
        # Output
        log_s = \
            f"\n<Tst Multi-hyp> - {time.time()-start_time:.3f}s - \n"\
            f"\tbleu:          {bleu:.5g}\n"\
            f"\tbow extrema:   {ext_emb_sim:.5g}\n"\
            f"\tbow avg:       {avg_emb_sim:.5g}\n"\
            f"\tbow greedy:    {greedy_emb_sim:.5g}\n"\
        ...
        scores = []
        idx = 0
        n_batches = math.ceil(len(mhyp_ctxs)/config.evaluator_batch_size)
        for batch_idx in tqdm(range(n_batches)):
            batch_ctx = mhyp_ctxs[batch_idx*config.evaluator_batch_size:(batch_idx+1)*config.evaluator_batch_size]
            batch_hyp = mhyp_hyps[batch_idx*config.evaluator_batch_size:(batch_idx+1)*config.evaluator_batch_size]
            batch_scores = evaluator.predict(batch_ctx, batch_hyp)
            scores += batch_scores
        evaluator_score = np.mean(scores)
        log_s = f"\n<Tst Multi-hyp> - {time.time()-start_time:.3f}s - evaluator score: {evaluator_score:.3f}\n"
        mlog(log_s)
